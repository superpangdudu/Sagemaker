...
max_height = os.environ.get("max_height", 1080)
max_width = os.environ.get("max_width", 1080)
...
model_config = {
    'base_model': {
        'name': 'watercolor_v5',
        'path': f's3://{bucket}/model/base/watercolor_v5',
...
for i in range(len(lora_configs)):
    ...
    logging.info(f'########## LoRA {lora_model_name} loading done')
...
def prepare_opt(input_data):
    ...
    opt["scheduler"] = input_data.get("scheduler", DEFAULT_SCHEDULER)
...
def predict_fn(input_data, m):
    ...
    try:
        ...
        num_images_per_prompt = input_data.get('count', 4)
        ...
        #
        scheduler = input_data.get('scheduler', DEFAULT_SCHEDULER)
